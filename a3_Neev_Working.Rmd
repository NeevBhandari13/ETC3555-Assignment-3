...
test_model <- function(model, test_df, train_df) {
  ...
  in_sample_rmse <- sqrt(mean((train_df$LOS - train_df$Predicted_LOS)^2))
  
  test_df$Predicted_LOS <- predict(model, newdata = test_df)
  
  out_of_sample_rmse <- sqrt(mean((test_df$LOS - test_df$Predicted_LOS)^2))
  
  print(paste("In-sample error (RMSE):", in_sample_rmse))
  print(paste("Out-of-sample error (RMSE):", out_of_sample_rmse))
}

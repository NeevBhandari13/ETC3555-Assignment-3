...
clean_df <- function(df) {
  ...
  df <- df %>%
    mutate(across(all_of(categorical_cols), as.factor))  # Convert to factors
  
  # Create one-hot encoding for categorical variables and keep numeric ones intact
...
```{r}
test_model <- function(model, test_df, train_df) {
  train_df$Predicted_LOS <- predict(model, newdata = train_df)
  
  in_sample_mse <- mean((train_df$LOS - train_df$Predicted_LOS)^2)
  
  test_df$Predicted_LOS <- predict(model, newdata = test_df)
  
  out_of_sample_mse <- mean((test_df$LOS - test_df$Predicted_LOS)^2)
  
  print(paste("In-sample error (MSE):", in_sample_mse))
  print(paste("Out-of-sample error (MSE):", out_of_sample_mse))
}

test_model(model, test_df, train_df)
...
```{r}
model_poly <- lm(LOS ~ poly(HeartRate_Mean, 2) + poly(SysBP_Mean, 2) + 
                       poly(RespRate_Mean, 2) + poly(TempC_Mean, 2) + poly(SpO2_Mean, 2) + 
                       poly(Glucose_Mean, 3) + GENDER + ADMISSION_TYPE + INSURANCE + 
                       RELIGION + MARITAL_STATUS + ETHNICITY + FIRST_CAREUNIT, data = train_df)

summary(model_poly)

test_model(model_poly, test_df, train_df)
...
```{r}
train_x <- model.matrix(~ . - 1, data = train_df %>% select(-LOS))   # Remove ID columns and target variable
train_y <- train_df$LOS

test_x <- model.matrix(~ . - 1, data = test_df %>% select(-LOS))    # Remove ID columns and target variable
test_y <- test_df$LOS

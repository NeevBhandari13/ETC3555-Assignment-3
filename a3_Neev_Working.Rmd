...
```{r}
train_x <- train_df %>% select(-LOS, -subject_id, -hadm_id, -icustay_id)  # Remove ID columns and target variable
train_y <- train_df$LOS

test_x <- test_df %>% select(-LOS, -subject_id, -hadm_id, -icustay_id)    # Remove ID columns and target variable
test_y <- test_df$LOS

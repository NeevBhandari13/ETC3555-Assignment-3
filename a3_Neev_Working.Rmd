...
# Adding L2 regularization to both hidden layer and output layer
nn %>% 
  layer_dense(units = 16, activation = 'relu', 
              input_shape = ncol(train_x), 
              kernel_regularizer = regularizer_l2(0.001)) %>%  # L2 regularization
  layer_dense(units = 1, activation = 'linear', 
              kernel_regularizer = regularizer_l2(0.001))  # L2 regularization for output
...
# Adding L2 regularization and Dropout for regularization
dnn %>% 
  # First hidden layer with 128 units, L2 regularization, and ReLU activation
  layer_dense(units = 128, activation = 'relu', 
              input_shape = ncol(train_x), 
              kernel_regularizer = regularizer_l2(0.001)) %>%
  layer_dropout(rate = 0.3) %>%  # Dropout to reduce overfitting
  
  # Second hidden layer with 64 units, L2 regularization, and ReLU activation
  layer_dense(units = 64, activation = 'relu', 
              kernel_regularizer = regularizer_l2(0.001)) %>%
  layer_dropout(rate = 0.3) %>%  # Another Dropout layer
  
  # Third hidden layer with 32 units, L2 regularization, and ReLU activation
  layer_dense(units = 32, activation = 'relu', 
              kernel_regularizer = regularizer_l2(0.001)) %>%
  
  # Output layer with 1 unit (for regression) and linear activation
  layer_dense(units = 1, activation = 'linear', 
              kernel_regularizer = regularizer_l2(0.001))  # L2 regularization for output
